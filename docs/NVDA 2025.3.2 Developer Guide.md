...
- [Translation](#translation)
  - [Character Descriptions](#character-descriptions)
    - [Translating this file](#translating-character-descriptions-file)
  - [Symbol Pronunciation](#symbol-pronunciation)
    - [Defining Complex Symbols](#defining-complex-symbols)
    - [Defining Symbol Information](#defining-symbol-information)
    - [Examples](#symbol-pronunciation-examples)
    - [Translating this file](#translating-symbols-file)
  - [Gestures](#gestures)
    - [Examples](#gesture-examples)
    - [How to remap a shortcut key](#how-to-remap-a-shortcut-key)
    - [Translating this file](#translating-gestures-file)
- [Plugins](#plugins)
  - [Overview](#overview)
  ...
  - [Example 3: an app module for an app employing Edge WebView2](#example-3-an-app-module-for-an-app-employing-edge-webview2)
  ...
  - [Scripts and Gesture Bindings](#scripts-and-gesture-bindings)
    - [Defining script properties](#defining-script-properties)
...
- [Packaging Code as NVDA Add-ons](#packaging-code-as-nvda-add-ons)
  ...
  - [Optional install / Uninstall code](#optional-install--uninstall-code)
    - [the onInstall function](#the-oninstall-function)
    - [The onUninstall Function](#the-onuninstall-function)
  - [Localizing Add-ons](#localizing-add-ons)
    - [Locale-specific Manifest Files](#locale-specific-manifest-files)
    - [Locale-specific Messages](#locale-specific-messages)
    - [Speech symbol dictionaries](#speech-symbol-dictionaries)
  - [Add-on Documentation](#add-on-documentation)
  - [Braille translation tables](#braille-translation-tables)
- [NVDA Python Console](#nvda-python-console)
  - [Usage](#python-console-usage)
  - [Namespace](#namespace)
    - [Automatic Imports](#automatic-imports)
    - [Snapshot Variables](#snapshot-variables)
  - [Tab completion](#tab-completion)
- [Remote Python Console](#remote-python-console)
  - [Usage](#remote-python-console-usage)
- [Extension Points](#extension-points)
  - [braille](#braille)
  - [appModuleHandler](#appmodulehandler)
  - [addonHandler](#addonhandler)
  - [brailleViewer](#brailleviewer)
  - [config](#config)
  - [core](#core)
  - [inputCore](#inputcore)
  ...
  - [utils.security](#utilssecurity)
  - [winAPI.messageWindow](#winapimessagewindow)
  - [winAPI.secureDesktop](#winapisecuredesktop)
  - [bdDetect](#bddetect)
...
The NVDA Add-on API changes over time, for example because of the addition of new features, removal or replacement of outdated libraries, deprecation of unused or replaced code and methodologies, and changes to Python. Important changes to the API are announced on the [NVDA API mailing list](https://groups.google.com/a/nvaccess.org/g/nvda-api/about). Changes relevant to developers are also announced via the [NVDA changes file](https://download.nvaccess.org/documentation/changes.html). Any changes to the API policy outlined in this section will be conveyed via these two channels.

API breaking releases happen at most once per year, these are `.1` releases, e.g. `2022.1`. The API remains backwards compatible between breaking releases. API breaking changes should be considered relatively stable in the first beta: e.g. `2022.1.beta1`.

API features may become deprecated over time. Deprecated API features may have a scheduled removal date, a future breaking release (e.g. `2022.1`). Deprecations may also have no scheduled removal date, and will remain supported until it is no longer reasonable. Note, the roadmap for removals is 'best effort' and may be subject to change. Please open a GitHub issue if the described add-on API changes result in the API no longer meeting the needs of an add-on you develop or maintain.

### A Note About Python

NVDA and its components are primarily written in the Python programming language. It is not the goal of this guide to teach you Python, though examples are provided through out this guide which will help to familiarise you with the Python syntax. Documentation and other resources related to the Python language can be found at [www.python.org/](https://www.python.org/)
...
#### Translating character descriptions file
...
The file contains two sections, [complex symbols](#defining-complex-symbols) and [symbols](#defining-symbol-information).
...
- On an Italian keyboard layout, `[` and `]` characters can only be input with the help of the `altGr` modifier: `altGr+e` and `altGr+plus`, respectively.

Thus, the Italian translators decided to remap these scripts using the two keys at the right of the `p` key on the Italian keyboard layout, i.e. `e` and `+`. To do this, they have added the following lines in the `gestures.ini` file:
...
1. In this file the sections correspond to the class to which the script belongs. If the class your looking for does not exist, create this section.

2. Under the targeted section, add a line corresponding to the new shortcut. e.g.:
...
3. If you want to unmap the original shortcut, just map it to `None`, e.g.:
...
Custom appModules and globalPlugins can be packaged into NVDA add-ons. This allows easy distribution, and provides a safe way for the user to install and uninstall the custom code. Please refer to the [Add-ons section](#packaging-code-as-nvda-add-ons) later on in this document.
...
Some executables host various apps inside or are employed by an app to display their interfaces. These include `javaw.exe` for running various Java programs, `wwahost.exe` for some apps in Windows 8 and later, and `msedgewebview2.exe` for displaying web-like interfaces on apps employing Edge WebView2 runtime.
...
For an example of an event handled by an App Module, please refer to [example 1](#example-1-an-app-module-that-beeps-on-focus-change-events) (focus beeps in notepad).
...
Add-ons make it easy for users to share and install plugins, drivers, speech symbol dictionaries and braille translation tables. They can be packaged in to a single NVDA add-on package, which the user can then install into a copy of NVDA via the Add-on Store found under Tools in the NVDA menu. An add-on package is simply a standard zip archive with the file extension of "`nvda-addon`". It can contain a manifest file, install/uninstall code and directories containing plugins, drivers, speech symbol dictionaries and braille translation tables.
...
- author (string, required): The author of this add-on, preferably in the form Full Name; e.g. Michael Curran <mick@example.com>.
...
- docFileName (string): The name of the main documentation file for this add-on; e.g. readme.html. See the [Add-on Documentation](#add-on-documentation) section for more details.
...
The manifest can also specify information regarding any additional speech symbol dictionaries or braille translation tables provided by the add-on. Please refer to the [speech symbol dictionaries](#speech-symbol-dictionaries) and [braille translation tables](#braille-translation-tables) sections.
...
- [Speech symbol dictionaries](#speech-symbol-dictionaries): Place them in the directory for one or more [locales](#localizing-add-ons) with a file name of `symbols-<name>.dic`, e.g. `locale\en\symbols-greek.dic`.
- [Braille translation tables](#braille-translation-tables): Place them in a `brailleTables` directory in the archive.
...
Note that for the display name of the dictionary to be translated, an entry should be added to a [locale manifest](#locale-specific-manifest-files). For example, add the following to `locale\fr\manifest.ini`:

```ini
[symbolDictionaries]
[[hebrew]]
displayName = Hebreu Biblique
```
...
Note that for the display name of the table to be translated, an entry should be added to a [locale manifest](#locale-specific-manifest-files). For example, add the following to `locale\fr\manifest.ini`:

```ini
[brailleTables]
[[no-no-8dot.utb]]
displayName = Norvegien Braille informatique 8 points - Remplacement
```
...
- By pressing NVDA+control+z. If activated in this fashion, a snapshot of the current state of NVDA at the time the key was pressed will be taken and saved in certain variables available in the console. See [Snapshot Variables](#snapshot-variables) for more details.
...
A remote Python console is available in source builds of NVDA, for situations where remote debugging of NVDA is useful. It is similar to the [local Python console](#nvda-python-console) discussed above, but is accessed via TCP.
...
The namespace is the same as [the namespace in the local Python console](#namespace).

There are some special functions:

- snap(): Takes a snapshot of the current state of NVDA and saves it in the [snapshot variables](#snapshot-variables).
...
| Type | Extension Point | Description |
|------|----------------|-------------|
| `AccumulatingDecider` | `isCLIParamKnown` | Allows adding NVDA commandline parameters which apply to plugins. See [Parsing additional command line arguments in your plugin](#parsing-additional-command-line-arguments-in-your-plugin) for more information. |
...
5. A dummy action that does nothing but close the dialog. In this case, and only this case, the return code from showing the dialog modally will be `EscapeCode.NO_FALLBACK`.
